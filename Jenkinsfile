pipeline{
				...
				steps{
					echo '------------>Analisis de código estático<------------'
					  withSonarQubeEnv('Sonar') {
                        sh "${tool name: 'SonarScanner', type: 'hudson.plugins.sonar.SonarRunnerInstallation'}/bin/sonar-scanner -Dsonar.projectKey=co.com.ceiba.estacionamiento.yesid.hoyos -Dsonar.projectName=Ceiba-Estacionamiento(yesid.hoyos) -Dproject.settings=./sonar-project.properties"
                     }
				}
			}
		...
		post {
			failure {
				mail(to: 'yesid.hoyos@ceiba.com.co',
